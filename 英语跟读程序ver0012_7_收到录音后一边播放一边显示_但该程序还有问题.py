...
class WhisperFollowReading:
    def __init__(self, model_size="tiny"):  # 改用更小的模型
        self.whisper_model = whisper.load_model(model_size)
        self.is_recording = False
        self.recording_thread = None
        self.frames = []
        self.sample_rate = 16000
        self.temp_file_counter = 0  # 添加计数器用于生成唯一文件名
        self.playback_audio = None  # 用于播放的音频
        self.transcribe_audio = None  # 用于转写的音频
        self.audio_queue = []  # 音频队列
        self.last_audio_time = 0  # 添加最后一次接收到音频的时间
        self.min_wait_time = 5  # 最短等待时间(秒)
        self.silence_threshold = 3  # 停顿阈值(秒)
        self.recognition_queue = []  # 添加识别队列
        self.is_processing = False

    def recognize_speech(self, audio_file):
        """改进的语音识别功能，使用单独的音频文件进行识别"""
        try:
            # 使用 Whisper 进行识别
    ...
    def save_audio_files(self):
        ...
        try:
            ...
            playback_file = f"temp_playback_{timestamp}.wav"
            transcribe_file = f"temp_transcribe_{timestamp}.wav"

            # 保存用于播放的音频
            self._save_wave_file(playback_file, self.frames)

            # 保存用于转写的音频（复制一份）
            self._save_wave_file(transcribe_file, self.frames)

            return playback_file, transcribe_file
        except Exception as e:
            logging.error(f"保存音频文件失败: {e}")
            messagebox.showerror("保存音频文件失败", f"保存失败: {str(e)}")
            return None, None
    ...
    def _save_wave_file(self, file_path, frames):
        ...
        try:
            ...
            messagebox.showerror("保存WAV文件失败", f"保存失败: {str(e)}")
            raise
...
class AudioPlayer:
    ...
    def cleanup_and_continue(self, playback_file, transcribe_file):
        ...
        try:
            ...
            pygame.mixer.music.unload()  # 取消加载音频文件
            ...
            try:
                ...
                # 如果仍然无法删除，记录警告但继续执行
                logging.warning(f"无法立即删除临时文件，将在程序关闭时清理")
                # 将文件添加到待清理列表
                if not hasattr(self, '_pending_cleanup_files'):
                    self._pending_cleanup_files = set()
                self._pending_cleanup_files.add(playback_file)
                self._pending_cleanup_files.add(transcribe_file)
        except Exception as e:
    ...
    def on_closing(self):
        ...
        try:
            ...
            # 清理待清理的临时文件
            if hasattr(self, '_pending_cleanup_files'):
                for file in self._pending_cleanup_files:
                    try:
