...
class AudioPlayer:
    ...
    def continue_after_playback(self):
        ...
        try:
            ...
            if next_segment < self.total_segments:
                # 当前音频还有下一段，继续播放
                self.follow_text.insert('end', "\n=== 准备下一段 ===\n", 'prompt')
                self.current_segment = next_segment  # 只有在确认要前进时才更新段落索引
                self.play_segment()
                return
    ...
    def pause_for_follow(self):
        ...
        if self.is_following:
            try:
                ...
                self.toggle_navigation_buttons(True)  # 确保在出错时启用导航按钮
                self.continue_after_playback()
    ...
    # self.current_position += pos / 1000.0 累加逻辑错误，导致位置异常增长
    def update_progress(self):
        ...
        try:
            ...
            if total_length > 0:
                ...
                if abs(progress - current_progress) > 0.5:  # 阈值可调整
                    self.progress_scale.set(progress)
    ...
    def on_progress_release(self, event):
        ...
        try:
            if self.current_playlist and self.is_playing:
                ...
                # 调用 seek_absolute 执行跳转
                self.seek_absolute(pos)
        except Exception as e:
            self.update_status(f"进度调整失败: {str(e)}", 'error')
        finally:
            self.is_seeking = False
            self.root.after(1000, self._resume_progress_update)  # 增加延迟到 1000ms
    ...
    def play_segment(self):
        ...
        try:
            ...
            logging.info(f"设置当前进度: {self.format_time(self.current_position)}")  # 添加日志
            self._update_tree_selection()
            ...
            self.progress_scale.set(progress)  # 手动设置初始进度
            ...
            if should_play_audio:
                ...
                if not self.load_audio_with_check(current_file):
                    logging.error("音频加载失败")  # 添加日志
                    self.update_status("加载音频失败", 'error')
                    self.is_playing = False  # 重置播放状态
                    self.is_playing_or_recording = False  # 重置状态
                    return
                logging.info(f"音频加载耗时: {time.time() - start_load:.2f}秒")
                ...
                pygame.mixer.music.play(start=start_time)  # start_time 单位是秒
                self.is_playing = True
                self.is_playing_or_recording = True  # 更新状态
                self.has_moved = False  # 重置切换标志
            ...
            self.is_playing_or_recording = False  # 重置状态
            if hasattr(self, '_follow_pause_timer') and self._follow_pause_timer:
                self.root.after_cancel(self._follow_pause_timer)  # 清理定时器
    ...
    def previous_segment(self):
        ...
        try:
            ...
            if hasattr(self, '_last_switch_time'):
                if time.time() - self._last_switch_time < 0.5:  # 限制最小切换间隔
                    return
            self._last_switch_time = time.time()

            # 如果正在播放/录音，且已经移动过，则忽略操作
