...
class SubtitleGeneratorWindow:
    def __init__(self, parent):
        ...
        ttk.Entry(self.path_frame, textvariable=self.audio_path_var).grid(row=0, column=1, padx=5, pady=5)
        ...
        ttk.Entry(self.path_frame, textvariable=self.output_path_var).grid(row=1, column=1, padx=5, pady=5)
        ...
        try:
            if os.path.exists(config_file):
                with open(config_file, 'r') as f:
                    config = json.load(f)
                    self.app_id_entry.insert(0, config.get('app_id', ''))
    ...
    def start_generate(self):
        ...
        if not app_id or not api_key:
            messagebox.showerror("错误", "请填写百度翻译API配置信息")
            return
        ...
        # 配置字幕生成器
        self.generator.set_api_config(app_id, api_key)
            ...
            try:
                ...
                for i, audio_file in enumerate(audio_files, 1):
                    try:
                        ...
                        # 生成字幕
                        if self.generator.generate_srt(audio_file, output_file):
                            success_count += 1
                            self.update_progress_text(f"✓ 成功: {os.path.basename(output_file)}")
                        else:
                            self.update_progress_text(f"✗ 失败: {os.path.basename(audio_file)}")

                    except Exception as e:
                        self.update_progress_text(f"✗ 处理出错: {str(e)}")
...
class WhisperFollowReading:
    ...
    def _save_wave_file(self, file_path, frames):
        ...
        try:
            ...
            if wave_file is not None:
                try:
                    # 使用 close() 之前先检查文件是否已关闭
                    if hasattr(wave_file, '_file') and wave_file._file is not None:
                        wave_file.close()
                except Exception as e:
                    logging.warning(f"关闭WAV文件时出现警告: {e}")
                    # 忽略关闭时的错误，因为文件可能已经被正确写入
                    pass
...
class AudioPlayer:
    ...
    def toggle_recording_mode(self):
        ...
        self.update_status(
            f"{'禁用' if self.no_record_mode else '启用'}录音模式",
            'info'
        )
    ...
    def toggle_playback_mode(self):
        ...
        self.update_status(
            f"{'禁用' if self.no_playback_mode else '启用'}跟读播放",
            'info'
        )
    ...
    def play_segment(self):
        ...
        try:
            ...
            if hasattr(self, 'is_following') and self.is_following:
                if hasattr(self, 'is_recording_mode') and self.is_recording_mode:
                    should_play_audio = not (hasattr(self, 'no_playback') and self.no_playback)
