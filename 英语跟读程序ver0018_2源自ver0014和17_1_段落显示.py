...
class AudioPlayer:
    ...
    def process_follow_reading(self):
        ...
        try:
            ...
            # 保存线程引用
            self._recognition_thread = threading.Thread(target=handle_recognition, daemon=True)
        ...
        try:
            ...
            for timer in ['update_timer', '_check_timer', '_playback_delay_timer']:
                if hasattr(self, timer):
                    timer_obj = getattr(self, timer)
                    if timer_obj:
                        self.root.after_cancel(timer_obj)
                        setattr(self, timer, None)
            print("4. 旧定时器已清理")  # 调试输出

            # 使用更低的更新频率
            self.update_timer = self.root.after(500, self.update_progress)  # 降低进度更新频率

            # 延迟启动播放状态检查
            self.root.after(1000, lambda: self._start_playback_check())  # 延迟1秒后开始检查
    ...
    def load_audio_with_check(self, file_path, timeout=5.0):
        """
        加载音频文件并检查加载状态。

        参数:
            file_path (str): 音频文件路径。
            timeout (float): 最大等待时间（秒），默认5秒。
        ...
        try:
            # 加载音频文件
            pygame.mixer.music.load(file_path)
            logging.info(f"开始加载音频文件: {file_path}")
            ...
            while time.time() - start_time < timeout:
                if pygame.mixer.music.get_busy():
                    logging.info("音频加载完成")
                    return True
                time.sleep(0.01)  # 短暂休眠，避免高 CPU 占用

            logging.error(f"音频加载超时 ({timeout}秒): {file_path}")
            self.update_status("音频加载超时", 'error')
            return False

        except Exception as e:
            logging.error(f"加载音频失败: {e}")
            self.update_status(f"加载音频失败: {str(e)}", 'error')
            return False
    ...
    def _process_switch_queue(self):
        ...
        if self._segment_switch_lock:
            # 如果锁未释放，延迟处理
            self.root.after(100, self._process_switch_queue)
            return

        if not self._segment_switch_queue:
            return

        operation, _ = self._segment_switch_queue.popleft()
    ...
    def play_segment(self):
        ...
        try:
            ...
            # 更新字幕显示
            # self.follow_text.delete('1.0', 'end')
            # self.follow_text.insert('end', f"=== 当前段落 {self.current_segment + 1}/{len(self.subtitles)} ===\n", 'title')
            ...
            # 更新当前位置
            self.current_position = start_time * 1000  # 保存当前位置（毫秒）
            ...
            if should_play_audio:
                try:
                    # 预加载音频
                    pygame.mixer.music.load(current_file)
                    time.sleep(0.5)  # 增加等待时间确保加载完成
                    # if not self.load_audio_with_check(current_file):
                    #     return  # 加载失败，直接返回，这个方法并不完全可靠，因为 get_busy() 主要用于检查音频是否正在播放，而不是加载状态。
                    ...
                    self._start_time = time.time() - start_time  # 为什么改？
                    self.is_playing = True

                except Exception as e:
                    logging.error(f"播放音频失败: {e}")
                    self.update_status("播放音频失败", 'error')
                    return

            # 7. 跟读模式处理
            if self.is_following:
                pause_time = int(duration * 1000)  # 仅用于播放音频的时间
    ...
    def previous_segment(self):
        ...
        try:
            if self._segment_switch_lock:
                self._segment_switch_queue.append(('previous', None))
                self.update_status("已加入切换队列，请稍候...", "info")
                return
            ...
            if self.current_segment > 0:
                ...
                self.follow_text.delete('1.0', 'end')
                self.follow_text.insert('end',
                                        f"准备播放第 {self.current_segment + 1}/{len(self.subtitles)} 段\n", 'prompt')
                self.follow_text.insert('end', "正在加载...\n", 'info')
                self.follow_text.see('end')
                self.root.update()  # 强制更新界面

                # 延迟一下再播放，确保显示更新
                # self.root.after(100, self.play_segment)
                # 直接调用理由：延迟可能导致顺序混乱，直接调用可以确保段落按顺序播放。
                self.play_segment()
                ...
                logging.info(f"切换到段落 {self.current_segment + 1} - 起始时间: {new_pos}s")
    ...
    def next_segment(self):
        ...
        try:
            if self._segment_switch_lock:
                self._segment_switch_queue.append(('next', None))
                self.update_status("已加入切换队列，请稍候...", "info")
                return
            self._segment_switch_lock = True
    ...
    def repeat_current_segment(self):
        ...
        try:
            if self._segment_switch_lock:
                self._segment_switch_queue.append(('repeat', None))
                self.update_status("已加入切换队列，请稍候...", "info")
                return
            self._segment_switch_lock = True
            ...
            if 0 <= self.current_segment < len(self.subtitles):
                ...
                logging.info(f"重复当前段落 {self.current_segment + 1} - 起始时间: {current_pos}s")
