...
class AudioPlayer:
    ...
    def update_progress(self):
        ...
        if self.is_playing and not self.is_seeking:
            try:
                ...
                if current_pos > 0:  # 只在有效播放时更新
                    current_pos = current_pos / 1000  # 转换为秒
                    total_length = self.get_current_audio_length()
        ...
        if self.is_following:
            ...
            if self.current_segment >= self.total_segments:
                ...
                if self.current_index < len(self.current_playlist) - 1:
                    ...
                    self.start_follow_reading()  # 重新开始跟读新的音频
                else:
                    self.stop_follow_reading()  # 所有文件都播放完毕
            return  # 这里应该有问题，也需要考虑循环次数
            ...
            self.info_label.config(text=f"当前播放: {current_file} ({self.current_loop}/{max_loops})")
            ...
            if self.current_loop < max_loops:
                self.play_current_track()  # 继续循环当前曲目
                return
    ...
    def play_current_track(self):
        ...
        try:
            self.current_loop = 0  # 重置当前循环计数
            self.max_loops = self.loop_count.get()  # 获取用户设置的循环次数
            ...
            self.info_label.config(text=f"当前播放: {current_file} (0/{self.max_loops})")
    ...
    def start_follow_reading(self):
        ...
        self.current_loop = 0  # 初始化循环计数
        self.max_follow_loops = self.loop_count.get()  # 获取设置的循环次数，只是做显示用吗？没看到实际循环
        ...
        self.follow_text.insert('end', f"开始跟读第 {self.current_index + 1} 个音频文件 (0/{self.max_follow_loops})\n")
        self.follow_text.see('end')
